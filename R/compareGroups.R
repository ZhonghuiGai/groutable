...
compareGroups <-
            ...
            riskratio = FALSE, riskratio.method = "wald", compute.prop = FALSE, lab.missing = "'Missing'")
  {
      ...
      if (inherits(formula, "matrix") || inherits(formula, "data.frame")){
        ...
        if (!is.null(y)){
          if (inherits(formula, "formula")){
            if (length(formula) > 2){ # response variable specified in formula and in y argument!!
    ...
    if (!all(pn %in% names(data))){
      pn <- sub("^`","",pn) # maybe there are some `name` in the formula terms to accommodate non standard characters
      pn <- sub("`$","",pn)
    }
